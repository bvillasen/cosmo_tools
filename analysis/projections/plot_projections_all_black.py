...
for i in range( n_cols):
  ...
  for n in range(n_rows):
    ...
    cb.ax.tick_params(labelsize=23, size=7, color=text_color, labelcolor=text_color)
    cb_fs = 25
    if i ==0:cb.set_label(cbar_labels[i], fontsize=cb_fs,  labelpad=-30, color=text_color)
    if i ==1:cb.set_label(cbar_labels[i], fontsize=cb_fs,  labelpad=-30, color=text_color)
    if i ==2:cb.set_label(cbar_labels[i], fontsize=cb_fs,  labelpad=-40, color=text_color)
    if i ==3:cb.set_label(cbar_labels[i], fontsize=cb_fs,  labelpad=-30, color=text_color )
