...
if field == 'density': input_dir =  dataDir + 'cosmo_sims/{0}_hydro_50Mpc/projections_pchw18/dens/projections_{1}_alpha_time_3/'.format(nPoints,size_front)
if field == 'temperature': input_dir =  dataDir + 'cosmo_sims/{0}_hydro_50Mpc/projections_pchw18/temp/projections_{1}_alpha_time_new/'.format(nPoints,size_front)
...
# if field == 'temperature': colorMap = 'turbo'
...
if get_statistics:
  ...
  file_pickle.close()
  current_z_list = np.array(current_z_list)
  np.savetxt( output_dir + 'z_vals.txt', current_z_list)
  exit(0)
...
for frame_index in indices_to_generate:
  # 
  # index_limit = 2038
  # if frame_index < index_limit: continue
  # if frame_index >= index_limit + 20: continue
  # 
  
  index_limit = 1200
  if frame_index < index_limit: continue
  if frame_index > 2400: continue
  # if frame_index != 1500: continue
  ...
  if normalize == 'global':
    ...
    if plot_field == 'distance':
      ...
      max_val = np.max( np.log10(statistics['distance']['max'])  ) * 0.92
    if plot_field == 'alpha':
