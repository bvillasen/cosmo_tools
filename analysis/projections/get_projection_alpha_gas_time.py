...
if field == 'density': output_dir =  dataDir + 'cosmo_sims/{0}_hydro_50Mpc/projections_pchw18/dens/projections_{1}_alpha_time_new/'.format(nPoints,size_front)
if field == 'temperature': output_dir =  dataDir + 'cosmo_sims/{0}_hydro_50Mpc/projections_pchw18/temp/projections_{1}_alpha_time_new/'.format(nPoints,size_front)
...
for i, index in enumerate(index_start_range): 

  index_limit = 1200
  if index < index_limit: continue
  # if index >= index_limit + 20: continue
  if index > 2400: continue
  ...
  nSnap_1 = nSnap+1
  if nSnap_1 > nSnap_0:
    data_snapshot = load_snapshot_data_distributed_periodix_x(  nSnap_1, inDir, data_type, field, subgrid, domain, precision, proc_grid, grid_complete_size, show_progess=show_progess )
    if data_type == 'particles': current_z = data_snapshot['current_z']
    if data_type == 'hydro': current_z = data_snapshot['Current_z']
    ...
    current_z =  current_z_0
    data =  data_0
    
  
  # if field == 'temperature':
  #   min_offset = 1000
  #   data = data - min_offset
  #   clip_max = None
  #   clip_min = 0
  #   data = np.clip(data, clip_min, clip_max)
