...
if field == 'density': output_dir =  dataDir + 'cosmo_sims/{0}_hydro_50Mpc/projections_pchw18/dens/projections_{1}_alpha_3/'.format(nPoints,size_front)
if field == 'temperature': output_dir =  dataDir + 'cosmo_sims/{0}_hydro_50Mpc/projections_pchw18/temp/projections_{1}_alpha_4/'.format(nPoints,size_front)
if field == 'HI_density': output_dir =  dataDir + 'cosmo_sims/{0}_hydro_50Mpc/projections_pchw18/HI_dens/projections_{1}_alpha_3/'.format(nPoints,size_front)
...
for i, indx_start in enumerate(index_start_range): 
  ...
  if field == 'HI_density': 
    data_dens = load_snapshot_data_distributed_periodix_x(  nSnap, inDir, data_type, 'density', subgrid, domain, precision, proc_grid, grid_complete_size, show_progess=show_progess )
    dens = data_dens[data_type]['density']
    data = data / dens
  ...
  #   clip_max = 34439390.0 / 10
  #   clip_min = 0
    # data = np.clip(data, clip_min, clip_max)
  if field == 'HI_density':
    # boost_factor = 1000000
    # data = data * boost_factor
    # clip_max = 0.053651143 * boost_factor * 10
    # clip_min = 0
    # data = np.clip(data, clip_min, clip_max)
    boost_factor = 10000000
    data = data * boost_factor
    clip_max = 1.0391423e-06 * boost_factor * 100
    clip_min = 0
    data = np.clip(data, clip_min, clip_max)
