...
output_dir = dataDir + 'cosmo_sims/2048_hydro_50Mpc/figures/transmited_flux/power_spectrum/cosmologies/'
...
cosmo_name_list = ['', '_cosmo_0', '_cosmo_1', '_cosmo_2', '_cosmo_3']
n_data = len(cosmo_name_list)


for index, cosmo_name in enumerate(cosmo_name_list):
  input_dir = dataDir + 'cosmo_sims/{0}_hydro_50Mpc{2}/transmited_flux_{1}/power_spectrum/multiple_axis/'.format(nPoints, uvb, cosmo_name)

  if cosmo_name == '':
    color_line = c_0
    color_bar = c_0
    label = "Planck18"  
    if nrows == 1: snapshots_indices = [83, 90,  96, 102 ]
    if nrows == 2: snapshots_indices = [ 119, 124, 130, 136, 143, 151, 159, 169, ]
    line_type = '-'
    lw = 3
  
  if cosmo_name == '_cosmo_0':
    color_line = 'C1'
    color_bar = 'C1'
    label = "COSMO A"
    if nrows == 1: snapshots_indices = [1, 2,  3, 4 ]
    if nrows == 2: snapshots_indices = [ 5, 6, 7, 8, 9, 151, 159, 169, ]
    line_type = '--'
    lw = 2
      
  if cosmo_name == '_cosmo_1':
    color_line = 'C4'
    color_bar = 'C4'
    label = "COSMO B"
    if nrows == 1: snapshots_indices = [1, 2,  3, 4 ]
  ...
  for snap_index, nSnap in enumerate(snapshots_indices):
    ...
    if plot_data_observed and index==n_data-1 :
      ...
      if diff_min < 1e-1:
        ...
        ax.errorbar( data_k, data_delta_power, yerr=data_delta_power_error, fmt='o', c=c_3, label='Walther+2018' )
      ...
      if diff_min < 1e-1:
        ...
        data_k = [100]
        data_delta_power = [100]
        data_delta_power_error = [100]
    ...
    x_min, x_max = 2e-3, 2e-1
    y_min, y_max = 4e-2, 1
    if plot_data_observed: y_min, y_max = 3e-2, 2
    ...
    if indx_j==0 :ax.text(0.27, 0.95, 'UVB=PCHW19', horizontalalignment='center',  verticalalignment='center', transform=ax.transAxes, fontsize=14, color=text_color) 
    ...
    legend_loc = 3
    if indx_i == nrows-1 and nrows!=2: legend_loc = 3
    if indx_j == ncols-1:
      leg = ax.legend( loc=legend_loc, frameon=False, fontsize=11)
...
if nrows == 1: fileName = output_dir + 'flux_power_spectrum_z5'
if nrows == 2: fileName = output_dir + 'flux_power_spectrum_z2'
...
if plot_data_observed: fileName += '_data'

  
fileName += '_cosmologies.png'
