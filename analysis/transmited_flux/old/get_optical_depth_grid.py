...
input_dir = dataDir + 'cosmo_sims/2048_hydro_50Mpc/outputs_files_{0}/'.format(uvb)
output_dir = dataDir + 'cosmo_sims/2048_hydro_50Mpc/optical_depth/'
...
for kernel_type in kernel_types:
  ...
  for n_box in range(n_boxes):
    ...
    F_val = F_vals.mean()
    data_out[kernel_type]['F_vals'].append( F_val )

    
  F_vals = np.array(data_out[kernel_type]['F_vals'])

  #Save Optical Depth data
  # group_kernel.create_dataset( 'tau_vals', data=tau_vals)
  group_kernel.create_dataset( 'F_vals', data=F_vals)
