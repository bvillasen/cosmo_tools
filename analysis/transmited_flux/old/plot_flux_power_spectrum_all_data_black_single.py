...
uvb = 'pchw18'
uvb = 'hm12'
output_dir = dataDir + 'cosmo_sims/{0}_hydro_50Mpc/figures/transmited_flux/power_spectrum/single_z/'.format(nPoints, uvb)
...
fig_single, ax_single = plt.subplots(nrows=nrows, ncols=ncols, figsize=(4*ncols,5*nrows))
...
for index,uvb in enumerate([ 'hm12', 'pchw18' ]):
  ...
  if uvb == 'pchw18':
    for i in range( n_lines ):
      ax_single.plot( k_vals ,  power_all[i*200]*k_vals)

    ax_single.set_yscale('log')
    ax_single.set_xscale('log')
    
    ax_single.text(0.85, 0.95, 'z={0:.1f}'.format(current_z), horizontalalignment='center',  verticalalignment='center', transform=ax_single.transAxes, fontsize=16) 


      
    
    ax_single.set_ylabel( r' $\Delta_F^2(k)$', fontsize=fs,  )
    ax_single.set_xlabel( r'$ k $    [s/km]', fontsize=fs, )
    fig_single.savefig( output_dir+ 'delta_F_samples.png',  pad_inches=0.1, bbox_inches='tight', dpi=200 )
...
if nrows == 1: fileName = output_dir + 'flux_power_spectrum_all_data_black_{0}.png'.format(nSnap)
if nrows == 2: fileName = output_dir + 'flux_power_spectrum_all_data_black_{0}.png'.format(nSnap)

if smooth:
  if nrows == 1: fileName = output_dir + 'flux_power_spectrum_all_data_black_smooth_{0}.png'.format(nSnap)
  if nrows == 2: fileName = output_dir + 'flux_power_spectrum_all_data_black_smooth_{0}.png'.format(nSnap)
  
  if transparent:
    if nrows == 1: fileName = output_dir + 'flux_power_spectrum_all_data_black_smooth_transparent_{0}.png'.format(nSnap)
    if nrows == 2: fileName = output_dir + 'flux_power_spectrum_all_data_black_smooth_transparent_{0}.png'.format(nSnap)
