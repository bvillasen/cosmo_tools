...
output_dir = dataDir + 'cosmo_sims/2048_hydro_50Mpc/figures/thermal_history/cosmo/'
...
#Load Alternative cosmologies

data_cosmo_all = []

n_cosmologies = 4

cosmo_names = ['cosmo_1', 'cosmo_2', 'cosmo_3', 'cosmo_4']

for i in range( n_cosmologies ):
  name_cosmo = 'cosmo_{0}'.format( i )
  ...
  for nSnap in range( 1, 16 ):
    ...
    mcmc_gamma = mcmc_stats['gamma']['mean']
    mcmc_gamma_sigma = mcmc_stats['gamma']['standard deviation']
    data_snapshot = [ mcmc_T0, mcmc_gamma, mcmc_T0_sigma, mcmc_gamma_sigma, current_z ]
    data_cosmo.append( data_snapshot)
  data_cosmo_all.append(data_cosmo)
data_cosmo_all = np.array(data_cosmo_all)
...
error_colors = [ c_err_0, c_err_1, c_err_2, c_err_3 ]
...
ax = ax_l[0]
alpha = 0.9
ax.fill_between( z_list, (T0_pchw18 + delta_T0_pchw18)/10**4, (T0_pchw18 - delta_T0_pchw18)/10**4, alpha=alpha, label='Planck', color=c_1 )
...
for i in range( n_cosmologies ):
  data_cosmo = data_cosmo_all[i]
  T0, gamma, sigma_T0, sigma_gamma, z = data_cosmo.T
  
  # Conver logT0 to T0
  T0 = 10**T0 
  delta_T0 =  T0 * np.log(10) * sigma_T0
  data_mean = T0 / 10**4
  data_error = delta_T0 / 10**4
  data_x = z

  # Convert gamma to gamma+1
  gamma = gamma + 1 
  
  ax.errorbar( data_x, data_mean, yerr=data_error, fmt='none',  alpha=1, ecolor= error_colors[i])
  ax.scatter( data_x, data_mean, label=cosmo_names[i], alpha=1, color= error_colors[i])
...
ax = ax_l[1]
ax.fill_between( z_list, gamma_pchw18 + sigma_gamma_pchw18, gamma_pchw18-sigma_gamma_pchw18, alpha=alpha, label='Planck', color=c_1)
for i,data_set in enumerate(data_sets):
  data_x = data_set['z']
...
fileName = output_dir + 'thermal_history_data+walther_cosmo_black.png'.format(nSnap)
...
# 
# ax = ax_l[0]
# ax.plot( z_list, T0_pchw18/10**4, label='Planck', c=c_1, lw=lw)
# ax.set_ylabel( r'$T_0$    [$10^4$ K]', fontsize=fs, color=text_color )
# ax.set_xlabel( 'Redshift', fontsize=fs, color=text_color )
# ax.set_xlim( 2, 16)
# leg = ax.legend( loc=1, frameon=False, fontsize=14)
...
# 
# ax = ax_l[1]
# ax.plot( z_list, gamma_pchw18, label='Planck', c=c_1, lw=lw)
# ax.set_ylabel( r'$\gamma $ ', fontsize=fs, color=text_color )
# ax.set_xlabel( 'Redshift', fontsize=fs, color=text_color )
# ax.set_xlim( 2, 16)
# leg = ax.legend( loc=1, frameon=False, fontsize=14)
...
# if not transparent: ax.set_facecolor('k')
# 
# fileName = output_dir + 'thermal_history_black_cosmo.png'.format(nSnap)
# if transparent: output_dir + 'thermal_history_transparent_cosmo.png'.format(nSnap)
# if not transparent: fig.savefig( fileName,  pad_inches=0.1, facecolor=fig.get_facecolor(),   bbox_inches='tight', dpi=300)
# else:  fig.savefig( fileName,  pad_inches=0.1, transparent=True,   bbox_inches='tight', dpi=300)
# print 'Saved Image: ', fileName
# 
