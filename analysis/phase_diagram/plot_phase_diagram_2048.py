...
output_dir = fit_mcmc_dir + 'figures/'
...
# min_global, max_global = np.Inf, -np.Inf
...
#   phase = phase[indices]
#   min_global = min( phase.min(), min_global)
#   max_global = max( phase.max(), max_global)
...
for nSnap in range( 170):
  ...
  im = ax.scatter( dens_points, temp_points, c=phase, s=0.1, vmin=-10, vmax=-3  )
  ...
  ax.fill_between( overdensity_values, temp_vals_r, temp_vals_l, facecolor='blue', alpha=0.5)

  title = r'$z = {0:.2f}$'.format( current_z ) 
  ax.text(0.75, 0.95, title, horizontalalignment='left',  verticalalignment='center', transform=ax.transAxes, fontsize=17)
