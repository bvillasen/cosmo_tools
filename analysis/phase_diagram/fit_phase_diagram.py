...
for nSnap in range(170):
  ...
  for index,overdensity_index in enumerate(overdensity_indices):
    ...
    max_val, sigma = get_max_and_sigma( temp_slice, centers_temp, output_dir=output_dir, plot_slice=True, index=index   )
  ...
  data_out = np.array([ overdensity_values, temp_mean_values, temp_sigma_values ])
